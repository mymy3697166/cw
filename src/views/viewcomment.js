...
import { View, FlatList, Image, Text, TouchableOpacity, MAINCOLOR, TextInput, Keyboard, LayoutAnimation } from '../';
...
export default class ViewComment extends ViewBase {
  ...
  onPublishPress() {
    ...
    this.post(this.urls.CREATE_COMMENT, {object_type: this.props.type, object_id: this.props.id, star: this.state.star, description: this.state.description}).then(e => {
      this.success('感谢您的参与');
      this.setState({
      ...
      this.n.broadcast('WALLPAPERUPDATESUCCESS', {id: this.props.id, type: this.props.type, comment_count: e.comment_count})
        ...
        <View style={{height: 96, borderTopWidth: this.px, borderTopColor: '#eee', backgroundColor: '#f1f1f1', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap'}}>
          <View style={{height: 48, width: this.fw, justifyContent: 'center', flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff'}}>
            ...
            <View style={{backgroundColor: '#eee', width: 200}}>
              <View style={{backgroundColor: MAINCOLOR, flexDirection: 'row', width: sw}}>
  ...
  renderHeader() {
    ...
    let b = 102 * p;
    return (
      <View style={{height: 96, marginTop: 8, flexDirection: 'row', flexWrap: 'wrap'}}>
        <Image
        ...
        <View style={{width: this.fw - 114}}>
          <View style={{backgroundColor: '#eee', height: 32, width: 200}}>
            <View style={{backgroundColor: MAINCOLOR, flexDirection: 'row', width: 200 * p}}>
              <Image source={require('../assets/star.png')} />
              <Image source={require('../assets/star.png')} />
              <Image source={require('../assets/star.png')} />
              <Image source={require('../assets/star.png')} />
          ...
          <View style={{height: 64, flexDirection: 'row', alignItems: 'flex-end', paddingLeft: 6}}>
            ...
            <Text style={{fontSize: 14, color: '#999', position: 'absolute', right: 0}}>{this.state.comment_count}人评论</Text>
          </View>
        </View>
        <View style={{backgroundColor: '#f1f1f1', height: 8, width: this.fw, marginTop: 8}}></View>
      </View>
    );
  }
