...
- (BOOL)tabBarController:(UITabBarController *)tabBarController shouldSelectViewController:(UIViewController *)viewController {
  ...
  if (tabBarController.selectedIndex != [tabBarController.viewControllers indexOfObject:viewController]) {
    NSDictionary *body = @{@"selectedTabIndex": @([tabBarController.viewControllers indexOfObject:viewController]), @"unselectedTabIndex": @(tabBarController.selectedIndex)};
...
- (void)centerButtonClicked:(UIButton *)sender {
  [RCCTabBarController sendScreenTabCenterButtonPressedEvent:self.selectedViewController body:nil];
}
...
- (instancetype)initWithProps:(NSDictionary *)props children:(NSArray *)children globalProps:(NSDictionary*)globalProps bridge:(RCTBridge *)bridge {
  ...
  if (tabsStyle) {
    id tabBarCenterButtonImage = tabsStyle[@"tabBarCenterButtonImage"];
    if (tabBarCenterButtonImage) {
      UIButton *cb = [UIButton buttonWithType:UIButtonTypeSystem];
      cb.bounds = CGRectMake(0, 0, 64, 64);
      [cb setImage:[RCTConvert UIImage:tabBarCenterButtonImage] forState:UIControlStateNormal];
      cb.backgroundColor = UIColor.whiteColor;
      cb.layer.cornerRadius = 32;
      cb.layer.shadowColor = UIColor.blackColor.CGColor;
      cb.layer.shadowOffset = CGSizeMake(0, 0);
      cb.layer.shadowRadius = 2;
      cb.layer.shadowOpacity = 0.2;
      [cb addTarget:self action:@selector(centerButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
      RCCTabBar *tb = [[RCCTabBar alloc] initWithCenterButton:cb];
...
- (void)performAction:(NSString*)performAction actionParams:(NSDictionary*)actionParams bridge:(RCTBridge *)bridge completion:(void (^)(void))completion {
  if ([performAction isEqualToString:@"setBadge"]) {
    ...
    if (tabIndex) {
      ...
      if ([self.viewControllers count] > i) viewController = [self.viewControllers objectAtIndex:i];
    }
    ...
    if (viewController) {
      ...
      if (badge == nil || [badge isEqual:[NSNull null]]) viewController.tabBarItem.badgeValue = nil;
      else viewController.tabBarItem.badgeValue = [NSString stringWithFormat:@"%@", badge];
    }
  }
  ...
  if ([performAction isEqualToString:@"setTabBarHidden"]) {
    ...
    [UIView animateWithDuration: ([actionParams[@"animated"] boolValue] ? 0.45 : 0) delay: 0 usingSpringWithDamping: 0.75 initialSpringVelocity: 0 options: (hidden ? UIViewAnimationOptionCurveEaseIn : UIViewAnimationOptionCurveEaseOut) animations:^() {
    ...
    return;
  } else if (completion != nil) completion();
}
...
+ (void)sendScreenTabCenterButtonPressedEvent:(UIViewController *)viewController body:(NSDictionary *)body {
  [RCCTabBarController sendTabEvent:@"bottomTabCenterButtonClicked" controller:viewController body:body];
}
...
+ (void)sendTabEvent:(NSString *)event controller:(UIViewController*)viewController body:(NSDictionary *)body {
  if ([viewController.view isKindOfClass:[RCTRootView class]]){
    ...
    if (rootView.appProperties && rootView.appProperties[@"navigatorEventID"]) {
      ...
      NSMutableDictionary *screenDict = [NSMutableDictionary dictionaryWithDictionary:@{@"id": event, @"navigatorID": navigatorID, @"screenInstanceID": screenInstanceID}];
      if (body) [screenDict addEntriesFromDictionary:body];
