...
- (instancetype)initWithProps:(NSDictionary *)props children:(NSArray *)children globalProps:(NSDictionary*)globalProps bridge:(RCTBridge *)bridge {
  ...
  if (leftButtons) [self setButtons:leftButtons viewController:viewController side:@"left" animated:NO];
  NSArray *rightButtons = props[@"rightButtons"];
  if (rightButtons) [self setButtons:rightButtons viewController:viewController side:@"right" animated:NO];
  self = [super initWithRootViewController:viewController];
  if (!self) return nil;
  self.delegate = self;
  self.navigationBar.translucent = NO; // default
  [self processTitleView:viewController props:props style:navigatorStyle];
...
- (void)performAction:(NSString*)performAction actionParams:(NSDictionary*)actionParams bridge:(RCTBridge *)bridge {
  ...
  if ([performAction isEqualToString:@"push"]) {
    ...
    [self processTitleView:viewController props:actionParams style:navigatorStyle];
    NSString *backButtonTitle = actionParams[@"backButtonTitle"];
    if (backButtonTitle) {
      UIBarButtonItem *backItem = [[UIBarButtonItem alloc] initWithTitle:backButtonTitle style:UIBarButtonItemStylePlain target:nil action:nil];
  ...
  if ([performAction isEqualToString:@"setTitle"] || [performAction isEqualToString:@"setTitleImage"]) {
    NSDictionary *navigatorStyle = actionParams[@"style"];
    [self processTitleView:self.topViewController props:actionParams style:navigatorStyle];
    return;
  }
  // toggleNavBar
...
- (void)setButtons:(NSArray*)buttons viewController:(UIViewController*)viewController side:(NSString*)side animated:(BOOL)animated {
  ...
  for (NSDictionary *button in buttons) {
    ...
    if (iconImage) barButtonItem = [[UIBarButtonItem alloc] initWithImage:iconImage style:UIBarButtonItemStylePlain target:self action:@selector(onButtonPress:)];
    else if (title) {
      ...
      if (buttonTextAttributes.allKeys.count > 0) [barButtonItem setTitleTextAttributes:buttonTextAttributes forState:UIControlStateNormal];
    } else if (component) {
      RCTBridge *bridge = [[RCCManager sharedInstance] getBridge];
      barButtonItem = [[RCCCustomBarButtonItem alloc] initWithComponentName:component passProps:button[@"passProps"] bridge:bridge target:self action:@selector(onButtonPress:)];
    } else continue;
    ...
    if (buttonId) objc_setAssociatedObject(barButtonItem, &CALLBACK_ASSOCIATED_ID, buttonId, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    NSNumber *disabled = button[@"disabled"];
    BOOL disabledBool = disabled ? [disabled boolValue] : NO;
    if (disabledBool) [barButtonItem setEnabled:NO];
    ...
    if (disableIconTint) [barButtonItem setImage:[barButtonItem.image imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal]];
    NSString *testID = button[@"testID"];
    if (testID) barButtonItem.accessibilityIdentifier = testID;
  }
  if ([side isEqualToString:@"left"]) [viewController.navigationItem setLeftBarButtonItems:barButtonItems animated:animated];
  if ([side isEqualToString:@"right"]) [viewController.navigationItem setRightBarButtonItems:barButtonItems animated:animated];
}

- (void)processTitleView:(UIViewController*)viewController props:(NSDictionary*)props style:(NSDictionary*)style {
  BOOL isSetSubtitleBool = props[@"isSetSubtitle"] ? [props[@"isSetSubtitle"] boolValue] : NO;
  RCCTitleViewHelper *titleViewHelper = [[RCCTitleViewHelper alloc] init:viewController navigationController:self title:props[@"title"] subtitle:props[@"subtitle"] titleImageData:props[@"titleImage"] isSetSubtitle:isSetSubtitleBool];
