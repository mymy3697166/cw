...
- (UIImageView *)navBarHairlineImageView {
  if (!_navBarHairlineImageView) _navBarHairlineImageView = [self findHairlineImageViewUnder:self.navigationController.navigationBar];
...
+ (UIViewController*)controllerWithLayout:(NSDictionary *)layout globalProps:(NSDictionary *)globalProps bridge:(RCTBridge *)bridge {
  ...
  if ([type isEqualToString:@"ViewControllerIOS"]) controller = [[RCCViewController alloc] initWithProps:props children:children globalProps:globalProps bridge:bridge];
  // navigation controller
  if ([type isEqualToString:@"NavigationControllerIOS"]) controller = [[RCCNavigationController alloc] initWithProps:props children:children globalProps:globalProps bridge:bridge];
  // tab bar controller
  if ([type isEqualToString:@"TabBarControllerIOS"]) controller = [[RCCTabBarController alloc] initWithProps:props children:children globalProps:globalProps bridge:bridge];
  // side menu controller
  if ([type isEqualToString:@"DrawerControllerIOS"]) {
    NSString *drawerType = props[@"type"];
    if ([drawerType isEqualToString:@"TheSideBar"]) controller = [[RCCTheSideBarManagerViewController alloc] initWithProps:props children:children globalProps:globalProps bridge:bridge];
    else controller = [[RCCDrawerController alloc] initWithProps:props children:children globalProps:globalProps bridge:bridge];
  }
  ...
  if (controller && componentId) {
    ...
    if([controller isKindOfClass:[RCCViewController class]]) ((RCCViewController*)controller).controllerId = componentId;
  }
...
-(NSDictionary*)addCommandTypeAndTimestampIfExists:(NSDictionary*)globalProps passProps:(NSDictionary*)passProps {
  ...
  if (commandType) modifiedPassProps[GLOBAL_SCREEN_ACTION_COMMAND_TYPE] = commandType;
  NSString *timestamp = globalProps[GLOBAL_SCREEN_ACTION_TIMESTAMP];
  if (timestamp) modifiedPassProps[GLOBAL_SCREEN_ACTION_TIMESTAMP] = timestamp;
...
-(void)onCancelReactTouches {
  if ([self.view isKindOfClass:[RCTRootView class]]) [(RCTRootView*)self.view cancelTouches];
}
...
- (void)sendScreenChangedEvent:(NSString *)eventName {
  if ([self.view isKindOfClass:[RCTRootView class]]){
    ...
    if (rootView.appProperties && rootView.appProperties[@"navigatorEventID"]) {
      [[[RCCManager sharedInstance] getBridge].eventDispatcher sendAppEventWithName:rootView.appProperties[@"navigatorEventID"] body:@{@"type": @"ScreenChangedEvent", @"id": eventName}];
    }
  }
}
...
- (void)sendGlobalScreenEvent:(NSString *)eventName endTimestampString:(NSString *)endTimestampStr shouldReset:(BOOL)shouldReset {
  ...
  if ([self.view isKindOfClass:[RCTRootView class]]){
    ...
    [[[RCCManager sharedInstance] getBridge].eventDispatcher sendAppEventWithName:eventName body:@{@"commandType": self.commandType ? self.commandType : @"", @"screen": screenName ? screenName : @"", @"startTime": self.timestamp ? self.timestamp : @"", @"endTime": endTimestampStr ? endTimestampStr : @""}];
    if (shouldReset) {
...
-(void)setStyleOnAppearForViewController:(UIViewController*)viewController appeared:(BOOL)appeared {
  ...
  } else viewController.navigationController.navigationBar.barTintColor = nil;
  ...
  if (navButtonTextAttributes.allKeys.count > 0 || leftNavButtonTextAttributes.allKeys.count > 0 || rightNavButtonTextAttributes.allKeys.count > 0) {
    for (UIBarButtonItem *item in viewController.navigationItem.leftBarButtonItems) {
      ...
      if (leftNavButtonTextAttributes.allKeys.count > 0) [item setTitleTextAttributes:leftNavButtonTextAttributes forState:UIControlStateNormal];
    }
    for (UIBarButtonItem *item in viewController.navigationItem.rightBarButtonItems) {
      [item setTitleTextAttributes:navButtonTextAttributes forState:UIControlStateNormal];
      if (rightNavButtonTextAttributes.allKeys.count > 0) [item setTitleTextAttributes:rightNavButtonTextAttributes forState:UIControlStateNormal];
    }
  ...
  } else viewController.navigationController.navigationBar.tintColor = nil;
  ...
  if (finalColorScheme && [finalColorScheme isEqualToString:@"light"]) {
    if (!statusBarTextColorSchemeSingleScreen) viewController.navigationController.navigationBar.barStyle = UIBarStyleBlack;
    self._statusBarTextColorSchemeLight = true;
    if (!viewControllerBasedStatusBarAppearance) [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleLightContent];
    [viewController setNeedsStatusBarAppearanceUpdate];
  } else {
    if (!statusBarTextColorSchemeSingleScreen) viewController.navigationController.navigationBar.barStyle = UIBarStyleDefault;
    self._statusBarTextColorSchemeLight = false;
    if (!viewControllerBasedStatusBarAppearance) [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleDefault];
    [viewController setNeedsStatusBarAppearanceUpdate];
  }
  NSNumber *navBarHidden = self.navigatorStyle[@"navBarHidden"];
  BOOL navBarHiddenBool = navBarHidden ? [navBarHidden boolValue] : NO;
  if (viewController.navigationController.navigationBarHidden != navBarHiddenBool) [viewController.navigationController setNavigationBarHidden:navBarHiddenBool animated:YES];
  ...
  if (navBarHideOnScrollBool) viewController.navigationController.hidesBarsOnSwipe = YES;
  else viewController.navigationController.hidesBarsOnSwipe = NO;
  ...
  if (!self.transitionCoordinator || self.transitionCoordinator.initiallyInteractive || !navBarTransparentBool || appeared) action();
  else {
  ...
  if (navBarTranslucentBool || navBarBlurBool) viewController.navigationController.navigationBar.translucent = YES;
  else viewController.navigationController.navigationBar.translucent = NO;
  ...
  if (drawUnderNavBarBool) viewController.edgesForExtendedLayout |= UIRectEdgeTop;
  else viewController.edgesForExtendedLayout &= ~UIRectEdgeTop;
  ...
  if (drawUnderTabBarBool) viewController.edgesForExtendedLayout |= UIRectEdgeBottom;
  else viewController.edgesForExtendedLayout &= ~UIRectEdgeBottom;
  ...
  if (removeNavBarBorderBool) self.navBarHairlineImageView.hidden = YES;
  else self.navBarHairlineImageView.hidden = NO;
...
-(void)storeOriginalNavBarImages {
  ...
  if (bgImage != nil) originalNavBarImages[@"bgImage"] = bgImage;
  UIImage *shadowImage = self.navigationController.navigationBar.shadowImage;
  if (shadowImage != nil) originalNavBarImages[@"shadowImage"] = shadowImage;
...
- (void)setStyleOnInit {
  ...
  if (tabBarHiddenBool) self._hidesBottomBarWhenPushed = YES;
  else self._hidesBottomBarWhenPushed = NO;
  ...
  if (statusBarHideWithNavBarBool) self._statusBarHideWithNavBar = YES;
  else self._statusBarHideWithNavBar = NO;
  ...
  if (statusBarHiddenBool) self._statusBarHidden = YES;
  else self._statusBarHidden = NO;
}
...
- (BOOL)prefersStatusBarHidden {
  if (self._statusBarHidden) return YES;
  if (self._statusBarHideWithNavBar) return self.navigationController.isNavigationBarHidden;
  else return NO;
}
...
- (UIStatusBarStyle)preferredStatusBarStyle {
  if (self._statusBarTextColorSchemeLight) return UIStatusBarStyleLightContent;
...
- (void)addExternalVCIfNecessary:(NSDictionary*)props {
  ...
  if (externalScreenClass != nil) {
    ...
    if (class != NULL) {
      ...
      } else NSLog(@"addExternalVCIfNecessary: could not create instance. Make sure that your class is a UIViewController whihc confirms to RCCExternalViewControllerProtocol");
    } else NSLog(@"addExternalVCIfNecessary: could not create class from string. Check that the proper class name wass passed in ExternalNativeScreenClass");
  }
}
...
- (NSString*) customNewRelicInteractionName {
  ...
  if (self.view != nil && [self.view isKindOfClass:[RCTRootView class]]) {
    ...
    if(moduleName != nil) interactionName = [NSString stringWithFormat:@"RCCViewController: %@", moduleName];
  }
  if (interactionName == nil) interactionName = [NSString stringWithFormat:@"RCCViewController with title: %@", self.title];
